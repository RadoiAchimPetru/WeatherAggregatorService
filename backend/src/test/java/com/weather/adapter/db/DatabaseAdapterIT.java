...
import com.weather.bdd.WireMockResource;
...
class DatabaseAdapterIT {
    ...
    void findLatestByCity_whenMultipleReadings_shouldReturnMostRecent() {
        ...
        databaseAdapter.save(new WeatherReading(
                "SortCity", 20.0, 5.0, "Fog", earlier));
        databaseAdapter.save(new WeatherReading(
                "SortCity", 25.0, 8.0, "Overcast", later));

        Optional<WeatherReading> latest =
                databaseAdapter.findLatestByCity("SortCity");
